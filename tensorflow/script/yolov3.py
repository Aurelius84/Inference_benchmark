...
def convolutional(input_layer,
                  ...
                  bn=True):
    if downsample:
        input_layer = tf.keras.layers.ZeroPadding2D(((1, 0),
                                                     (1, 0)))(input_layer)
        padding = 'valid'
        strides = 2
    else:
...
def residual_block(input_layer, input_channel, filter_num1, filter_num2):
    ...
    conv = convolutional(
        input_layer, filters_shape=(1, 1, input_channel, filter_num1))
...
def YOLOv3(input_layer):
    ...
    conv_lbbox = convolutional(
        conv_lobj_branch, (1, 1, 1024, 3 * (NUM_CLASS + 5)),
        activate=False,
        bn=False)
    ...
    conv_mbbox = convolutional(
        conv_mobj_branch, (1, 1, 512, 3 * (NUM_CLASS + 5)),
        activate=False,
        bn=False)
    ...
    conv_sbbox = convolutional(
        conv_sobj_branch, (1, 1, 256, 3 * (NUM_CLASS + 5)),
        activate=False,
        bn=False)
...
def decode(conv_output, i=0):
    ...
    y = tf.tile(
        tf.range(
            output_size, dtype=tf.int32)[:, tf.newaxis], [1, output_size])
    x = tf.tile(
        tf.range(
            output_size, dtype=tf.int32)[tf.newaxis, :], [output_size, 1])
